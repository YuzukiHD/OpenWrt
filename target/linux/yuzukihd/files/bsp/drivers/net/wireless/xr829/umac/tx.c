...
ieee80211_tx_h_multicast_ps_buf(struct ieee80211_tx_data *tx)
{
	...
	if (skb_queue_len(&ps->bc_buf) >= AP_MAX_BC_BUFFER) {
		...
		mac80211_free_txskb(&tx->local->hw, skb_dequeue(&ps->bc_buf));
	} else
	...
	if (unlikely((test_sta_flag(sta, WLAN_STA_PS_STA) ||
		...
		if (skb_queue_len(&sta->ps_tx_buf[ac]) >= STA_MAX_TX_BUFFER) {
			...
			ps_dbg(tx->sdata,
			       ...
			       sta->sta.addr, ac);
			mac80211_free_txskb(&local->hw, old);
		} else
			tx->local->total_ps_buffered++;
				  ...
				  int tid)
{
	...
	if (test_bit(HT_AGG_STATE_OPERATIONAL, &tid_tx->state)) {
		...
		if (purge_skb)
			mac80211_free_txskb(&tx->local->hw, purge_skb);
	}
...
#define CALL_TXH(txh) \
 ...
 txh_done:
	if (unlikely(res == TX_DROP)) {
		...
		if (tx->skb)
			mac80211_free_txskb(&tx->local->hw, tx->skb);
		else
...
static void ieee80211_set_csa_proberesp(struct ieee80211_sub_if_data *sdata,
				struct proberesp_data *beacon)
{
	struct probe_resp *resp;
	u8 *beacon_data;
	size_t beacon_data_len;
...
#ifdef PROBE_RESP_EXTRA_IE
static u8 __ieee80211_csa_update_counter_proberesp(struct proberesp_data *beacon)
{
	beacon->csa_current_counter--;
						   ...
						   struct ieee80211_vif *vif)
{
	...
	if (sdata->vif.type == NL80211_IFTYPE_AP) {
		...
		if (proberesp) {
			if (proberesp->csa_counter_offsets[0]) {
					__ieee80211_csa_update_counter_proberesp(proberesp);
				/*need to set sdata->u.ap.proberesp */
				ieee80211_set_csa_proberesp(sdata, proberesp);
			}
			/*
			 * headroom, head length,
...
int mac80211_reserve_tid(struct ieee80211_sta *pubsta, u8 tid)
{
...
void mac80211_unreserve_tid(struct ieee80211_sta *pubsta, u8 tid)
{
	struct sta_info *sta = container_of(pubsta, struct sta_info, sta);
	struct ieee80211_sub_if_data *sdata = sta->sdata;
